...
function filaPorFiltros($datos, $condiciones) {
    foreach ($datos["rows"] as $i => $row) {
        ...
        if ($cumple) {
            return $i + 2; // fila real (1=headers, +1 por índice base 0)
        }
    }
    return null;
}
...
try {
    switch ($method) {
        ...
        case 'PUT': // UPDATE
            ...
            $valoresActuales = $datos["rows"][$fila - 2];
            $valoresActualizados = [];
            foreach ($datos["headers"] as $i => $header) {
                if (array_key_exists($header, $input)) {
                    $valoresActualizados[] = $input[$header];
                } else {
                    $valoresActualizados[] = $valoresActuales[$header] ?? "";
                }
            }
